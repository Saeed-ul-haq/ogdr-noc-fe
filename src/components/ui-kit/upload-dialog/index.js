...
const UploadProgressDialog = props => {
  const { percent = 0, closeUploadDialog, uploadedList = [] } = props
  return (
      ...
      <ExpansionList>
        ...
        >
          {uploadedList.map((m, index) => (
            <div key={index} className="single-upload-info-item">
              ...
              <div className="percentage-container">
                <div className="file-label">{m.name}</div>
                ...
                <div className="upload-message">
                  ...
                  <div className="file-size">{getFileSize(m.size)}</div>
                </div>
              </div>
            </div>
          ))}
